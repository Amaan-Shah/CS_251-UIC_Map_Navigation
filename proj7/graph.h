...
class graph {
 ...
 public:
  ...
  bool addEdge(VertexT from, VertexT to, WeightT weight) {
    if (AdjList.count(from) == 0 || AdjList.count(to) == 0) {  // O(1)
        return false;
    }
    // insert / update the edge:
    if (AdjList.at(from).count(to) == 0) {  // O(1)
        AdjList.at(from).emplace(to, weight);  // O(1)
        NeighborsList.at(from).emplace(to);  // O(1)
        edges++;
    } else {
        AdjList.at(from).at(to) = weight;  // O(1)
    }
    return true;
  }
  ...
  bool getWeight(VertexT from, VertexT to, WeightT& weight) const {
    if (AdjList.count(from) == 0) {  /// O(1)
        return false;
    } else if (AdjList.count(to) == 0) {  // O(1)
        return false;
    }
    if (AdjList.at(from).count(to) != 1) {  // O(1)
      return false;
    }
    weight = AdjList.at(from).at(to);  // O(1)
    return true;
  }
